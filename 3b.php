...
$solar_capacity = 2239;       // Model a 4kW system
...
{    
    ...
    if ($hour_in_day>=21.5 && $hour_in_day<(21.5+($shower_duration/3600))) {
        $electric_shower = 4184 * $shower_flowrate * ($shower_temperature-$inlet_temperature);
    }
    
    // -------------------------------------------------------------------------------
    // TOTAL CONSUMPTION BEFORE HOT WATER STORE DIVERT
    // -------------------------------------------------------------------------------
    $use = $centralheating_standby + $router + $monitoring + $lighting + $laptop1 + $laptop2 + $fridge + $washing + $kettle + $hob + $electric_shower;
